...
- **⚠️ Verify the G0B1's chosen I2C pins are 5V-tolerant (FT)**, and
  specifically whether tolerance holds with VDD off, against the datasheet's
  pin definition table. If it does not, the fallback is a 3.3V DVCC generated
  on the bus board.
...
- **The bootloader uses a fixed I2C address**, not the slot address, so **only
  ...
  strictly one-by-one. Verify the address against AN2606.
