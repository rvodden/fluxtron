...
### ⚠️⚠️ Open: 5V DVCC may not be viable at all

Being checked against the G0B1 datasheet (LQFP-48, `STM32G0B1CBT6`). ST's own
guidance is that FT tolerance **depends on VDD being present** — the structure
is a level shifter referenced to VDD, not a simple clamp — so with VDD = 0 and
5V on the pin, injection current flows and can damage the part.

**Our topology guarantees that condition.** Every module's 3.3V rail is derived
*from* the 5V rail, so 5V is necessarily up first: a window exists on **every
power cycle**, and a module whose 5V PTC trips or whose LDO fails sits
unpowered **indefinitely** with the bus live across its pins.

**Series resistors do not resolve it.** Against a typical ±5mA injection limit:

| Series R | Injection when unpowered | Bus low level |
|---|---|---|
| 220Ω | ~19.5mA — about 4× the limit | 0.65V ✓ |
| 860Ω (to reach 5mA) | 5mA ✓ | ~1.4V against a 1.5V VIL — no margin |

Injection protection and working I2C levels are mutually exclusive here.

**Recommendation: revert DVCC to 3.3V**, generated by a small LDO on PS-1's
bus board feeding only the pull-ups (a few mA). The bus then matches every MCU
natively, 400kHz comes back (see below), and the unpowered case degrades to the
ordinary mixed-power situation every I2C design already handles. The case for
5V was ribbon noise margin alone, and it has now cost two things.

**Not yet decided — do not lay out a bus connector against 5V DVCC.**
