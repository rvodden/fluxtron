...
- **Channel/division encoder**: off-board entirely, on flying leads (A, B,
  ...
  provides all the mechanical support. **No cutout is needed**: the PEC11R
  sits 6.5mm behind the panel, wholly in front of the 10mm main PCB, which
  retires the clearance-hole worry the PEC16 carried. The switch pair is
  now used: push reaches the MIDI channel, which the knob otherwise
  does not touch.
  - **A height keep-out does still apply.** MC-1's encoder is on flying
    leads with the main PCB running full height behind it, so the only
    clearance is that 3.5mm — and the rear-facing PC pins and their
    soldered leads eat much of it. Keep tall components out of the
    footprint behind the encoder and insulate the joints so they cannot
    touch the board. (Modules whose encoders sit on a sub-board dodge this,
    because their main PCB stops short of the encoder block entirely.)
